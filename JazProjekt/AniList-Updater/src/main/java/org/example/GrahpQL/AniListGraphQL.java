...
public class AniListGraphQL {
    public static void main(String[] args) {
        ...
        for (int i = 1; i <= 500; i++) {
            ...
            try {
                ...
                Anime anime = parseGraphQLResponse(response, em);
    ...
    public static Anime parseGraphQLResponse(String response, EntityManager em) throws IOException {
        ...
        JsonNode studiosNode = mediaNode.path("studios").path("nodes");
        for (JsonNode studioNode : studiosNode) {
            int studioId = studioNode.path("id").asInt();
